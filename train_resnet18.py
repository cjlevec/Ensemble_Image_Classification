...
def main():
    ...
    # ResNet was previously trained on ImageNet, this uses those weights as a starting point
    model = resnet18(pretrained=pre_train)
    # Adjust final layer for 100 classes
    ...
    for epoch in range(n_epochs):
        ...
        if epoch == 4: # (account for fact that the epoch loop started at 0)
            torch.save(model.state_dict(), "resnet18_5_epochs.pth")
            print("Checkpoint saved at 5 epochs as resnet18_5_epochs.pth")

    # Save the final model parameters after full training
    torch.save(model.state_dict(), "resnet18_final.pth")
    print("Model training complete and saved as resnet18_final.pth")

    # Plot the loss curves for training and validation
    plt.plot(train_losses, label="Training Loss")
    plt.plot(val_losses, label="Validation Loss")
    plt.title("Training and Validation Loss for ResNet18 over 30 Epochs")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig("resnet18_train_val_loss.png")
    plt.show()

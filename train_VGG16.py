...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        if epoch == 4: # (account for fact that the epoch loop started at 0)
            torch.save(model.state_dict(), "vgg16_5_epochs.pth")
            print("Checkpoint saved at 5 epochs as vgg16_5_epochs.pth")

    # Save the final model parameters after full training
    torch.save(model.state_dict(), "vgg16_final.pth")
    print("Model training complete and saved as vgg16_final.pth")

    # Plot the loss curves for training and validation
    plt.plot(train_losses, label="Training Loss")
    plt.plot(val_losses, label="Validation Loss")
    plt.title("Training and Validation Loss for VGG16 over 30 Epochs")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
